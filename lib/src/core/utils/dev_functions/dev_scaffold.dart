...
class DevScaffold extends ConsumerStatefulWidget {
  /// Scaffold for development. Has EndDrawer with necessary development tools.
  const DevScaffold({
    super.key,
    required this.child,
  ...
  @override
  ConsumerState<DevScaffold> createState() => _DevScaffoldState();
}

class _DevScaffoldState extends ConsumerState<DevScaffold> {
          ...
          children: <Widget>[
            ...
            for (final ThemeMode themeMode in ThemeMode.values)
              _onPress(
                'Theme - $themeMode',
                () async {
                  await ref.read(themeProvider.notifier).updateTheme(themeMode);
                  return true;
                },
              ),
